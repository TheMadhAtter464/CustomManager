                  ...
                  @include('errors.error')
                    <form method="post" action="{{ route('post.update', $post->id) }}">
                      ...
                      <div class="form-group">
                        ...
                        <textarea rows="5" class="form-control" name="descr">{{$post->descr}}</textarea>
                      </div>
...
@section('after_scripts')
  ...
  <script type="text/javascript">
    ...
    $('.select2-multi').select2().val({!! json_encode($post->categories()->pluck('categories.id')->toArray()) !!}).trigger('change');
	</script>
